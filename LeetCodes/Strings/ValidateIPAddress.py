...
a valid one. Also, we could omit some leading zeros among four hexadecimal digits and some low-case characters in
the address to upper-case ones, so 2001:db8:85a3:0:0:8A2E:0370:7334 is also a valid IPv6 address(Omit leading zeros
...
class Solution(object):
    def validIPAddress(self, IP):
        ...
        """
        def isHex(s):
            candidates = set("0123456789abcdefABCDEF")
            for c in s:
        ...
        if len(fields) == 4:
            for field in fields:
                if not field.isdigit() or not 0 <= int(field) < 256 or ( field[0] == '0' and len(field) > 1):
                    return "Neither"
            return "IPv4"
        fields = IP.split(":")
        if len(fields) == 8:
            for field in fields:
                if len(field) == 0 or not isHex(field) or len(field) > 4:
                    return "Neither"
            return "IPv6"
        return "Neither"

res = Solution().validIPAddress("2001:0db8:85a3:0:0:8A2E:0370:7334")
print(res)