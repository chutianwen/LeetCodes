...
class Solution(object):
    def lengthLongestPath(self, input):
        """
        Notice that a = '\t' is only length 1
        Use dict to store key: depth, value:#path
        Abs_path = path_dir + file_name
        :type input: str
        :rtype: int
        """
        par_length = {0: 0}
        ...
        for line in input.split('\n'):
            ...
            depth = len(line) - len(line_stripped)
            if '.' in line_stripped:
                max_abs_path = max(max_abs_path, par_length[depth] + len(line_stripped))
            else:
                par_length[depth + 1] = par_length[depth] + len(line_stripped) + 1
        return max_abs_path